...
export async function fetchAll(groupId) {
  const [members, avail, hols, abr] = await Promise.all([
    ...
    supabase.from("abroad").select("name,year,dates").eq("group_id", groupId),
  ]);

  const data = { members: [], years: {} };
  data.members = (members.data || []).map((r) => r.name);
  const ensure = (y) => {
    if (!data.years[y]) data.years[y] = { holidays: [], avail: {}, abroad: {} };
    return data.years[y];
  };

  for (const row of avail.data || []) ensure(row.year).avail[row.name] = row.dates || [];
  for (const row of hols.data || []) ensure(row.year).holidays = row.dates || [];
  for (const row of abr.data || []) ensure(row.year).abroad[row.name] = row.dates || [];
  return data;
}
...
export async function saveAbroad(groupId, name, year, dates) {
  await supabase.from("abroad").upsert({
    group_id: groupId, name, year, dates, updated_at: new Date().toISOString(),
  });
}
